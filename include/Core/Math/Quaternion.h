...
public:
    ...
    Math::Matrix3 ToRotationMatrix() const {
        // 若未归一化，可先归一化当前四元数
    ...
    static Quaternion fromAxisAngle(const Vector3& axis, Scalar angle) {
...
inline Quaternion operator*(Scalar scalar, const Quaternion& q) {
    return q * scalar;
}
